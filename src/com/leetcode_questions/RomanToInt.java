...
public class RomanToInt {
	public static int romanToInt(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			if (i < s.length() - 1 && (num.get(s.charAt(i)) < num.get(s.charAt(i + 1)))) {
				result += num.get(s.charAt(i + 1)) - num.get(s.charAt(i));
				System.out.println("if res is :" + result);
				i++;
			} else {
	...
	public static void main(String[] args) {
		...
		String s = "LVIII";
//     System.out.println("Roman to integer is :"+romanToInt(s));
		System.out.println("Roman to integer is :" + romanToInt("MCMXCIV"));
